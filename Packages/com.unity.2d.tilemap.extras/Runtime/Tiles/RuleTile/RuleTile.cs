...
namespace UnityEngine
{
    ...
    public class RuleTile : TileBase
    {
        ...
        public static bool IsTilemapUsedTilesChange(Tilemap tilemap, out KeyValuePair<HashSet<TileBase>, HashSet<Vector3Int>> hashSet)
        {
        ...
        public static KeyValuePair<HashSet<TileBase>, HashSet<Vector3Int>> CachingTilemapNeighborPositions(Tilemap tilemap)
        {
        ...
        public static void ReleaseDestroyedTilemapCacheData()
        {
            if (!NeedRelease())
                return;
        ...
        /// </summary>
        /// <param name="tileLocalPos">Position of the Tile on the Tilemap.</param>
        /// <param name="tilemap">The Tilemap the tile is present on.</param>
        public override void RefreshTile(Vector3Int tileLocalPos, ITilemap tilemap)
        {
            base.RefreshTile(tileLocalPos, tilemap);
        ...
        /// <param name="localPosition">Position of the Tile on the Tilemap.</param>
        ...
        public virtual bool RuleMatches(TilingRule rule, Vector3Int localPosition, ITilemap tilemap, ref Matrix4x4 transform)
        {
            if (RuleMatches(rule, localPosition, tilemap, 0))
            {
            ...
            if (rule.m_RuleTransform == TilingRuleOutput.Transform.Rotated)
            {
                for (int angle = m_RotationAngle; angle < 360; angle += m_RotationAngle)
                {
                    if (RuleMatches(rule, localPosition, tilemap, angle))
                    {
            ...
            {
                if (RuleMatches(rule, localPosition, tilemap, true, true))
                {
                    transform = Matrix4x4.TRS(Vector3.zero, Quaternion.identity, new Vector3(-1f, -1f, 1f));
                    return true;
                }
                if (RuleMatches(rule, localPosition, tilemap, true, false))
                {
                    transform = Matrix4x4.TRS(Vector3.zero, Quaternion.identity, new Vector3(-1f, 1f, 1f));
                    return true;
                }
                if (RuleMatches(rule, localPosition, tilemap, false, true))
                {
                    transform = Matrix4x4.TRS(Vector3.zero, Quaternion.identity, new Vector3(1f, -1f, 1f));
                    return true;
                }
        ...
        /// <returns>True if there is a match, False if not.</returns>
        public bool RuleMatches(TilingRule rule, Vector3Int localPosition, ITilemap tilemap, int angle)
        {
            ...
            for (int i = 0; i < minCount ; i++)
            {
                ...
                TileBase other = tilemap.GetTile(GetOffsetPosition(localPosition, positionOffset));
        ...
        /// <returns>True if there is a match, False if not.</returns>
        public bool RuleMatches(TilingRule rule, Vector3Int localPosition, ITilemap tilemap, bool mirrorX, bool mirrorY)
        {
            var minCount = Math.Min(rule.m_Neighbors.Count, rule.m_NeighborPositions.Count);
            for (int i = 0; i < minCount; i++)
            {
